...
def create_env(cfg, agent: str, config_file: str, scene_id: str, control_freq: float, cheap: bool, seed: int) -> LLMEnv:
    ...
    llm = LLM_hugging(debug=True, room_classification_model="gpt-3.5-turbo-1106", open_set_rooms=cfg["open_set_room_categories"],)
...
def evaluate_scene(config_file: str, cfg, scene_id: str, tot_ep: int) -> list:
    ...
    for i in range(cfg["num_episodes_per_scene"]):
        ...
        high_level_env.llm.save_PPO_checkpoint(save_name=f"Qwen-SFT-unseen-4epochs/Checkpoint_PPO_{scene_id}_eps_{i}")
...
def main():
    ...
    wandb.init(project="[scene-llm-train]", 
               entity="quangpham200602", 
               config=cfg,
