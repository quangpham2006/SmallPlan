...
class LLMEnv(HighLevelEnv):
    def __init__(self, env: OurIGibsonEnv, llm, seed: int) -> None:
        # print(env)
    ...
    def compute_reward(self, env_feedback, obs, new_obs):
        #Compute reward if the sub-task is success:
        alpha_subtask = 0.3 #success or not
        beta_exploration = 0.1 #Encourage exploration of new areas 
        gamma_distance_travel = 0.3 # 0.6 #0.2 #Efficience
        # eta_step_taken = 3 #Efficience
        # np.linalg.norm(np.diff(np.stack(self.robot_traj)[:, :2, 3], axis=0), axis=-1).sum()
        
        if env_feedback["success"] == False:
            print("---------------Reward------------------: ", -alpha_subtask)
        ...
        reward_explore = beta_exploration*(len(new_obs["separated_voronoi_graph"]) - len(obs["separated_voronoi_graph"]))/20
