"""
Action Executor for High-Level Navigation Actions

Converts high-level actions (goto, explore, open, stop) to navigation.
Aligned with train_from_simulation_habitat smooth waypoint-based navigation.
"""
...
class ActionExecutor:
    ...
    def execute(self, action: HighLevelAction, observation) -> ActionResult:
        ...
        Aligned with train_from_simulation_habitat action execution:
        - goto(target): Navigate to object or room using smooth teleportation
        ...
        if action.name == "stop":
            result = self._execute_stop(observation)
        elif action.name == "goto":
            result = self._execute_goto(action.argument, observation)
        elif action.name == "explore":
            result = self._execute_explore(observation, room_name=action.argument)
        elif action.name == "open":
            result = self._execute_open(action.argument, observation)
        else:
    ...
    def _execute_stop(self, observation) -> ActionResult:
        """Execute stop action."""
    ...
    def _execute_goto(self, target: str, observation) -> ActionResult:
        """
        Navigate to the nearest visible target object using smooth navigation.
        
        Aligned with train_from_simulation_habitat goto logic:
        ...
        if not target_objects:
            ...
            return ActionResult(
                False, 0, 0.0, 
                f"Target '{target}' not visible. Visible: {visible}. Try explore().",
                observation=observation
            )
        ...
        if target_obj.distance < self.config.success_distance:
            return ActionResult(
                True, 0, 0.0,
                f"Already near {target} ({target_obj.distance:.1f}m). Use stop() if done.",
                observation=observation
            )
    ...
    def _execute_explore(self, observation, room_name: str = "") -> ActionResult:
        ...
        if distance < 0.1 and nav_distance < 0.5:
            objects_str = ", ".join(sorted(discovered_objects)[:10]) if discovered_objects else "none"
            room_info = f" in {room_name}" if room_name else ""
            feedback = f"No unexplored areas found{room_info}. Visible: {objects_str}. Try goto() or open() a door."
            success = False
        else:
            objects_str = ", ".join(sorted(discovered_objects)[:10]) if discovered_objects else "none"
            room_info = f" in {room_name}" if room_name else ""
    ...
    def _execute_open(self, target: str, observation) -> ActionResult:
        """
        ...
        if target_obj.distance > 2.0:
            ...
            if target_obj is None or target_obj.distance > 3.0:
                ...
                return ActionResult(
                    ...
                    f"Cannot reach {target} (still {dist_str} away). Try goto() first.",
                    observation=observation
                )
