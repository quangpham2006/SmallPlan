...
class LLMAgent(BaseAgent):
    ...
    Uses a language model (via API) to make navigation decisions.
    Supports two action levels:
    - "low": Low-level actions (MOVE_FORWARD, MOVE_BACKWARD, TURN_LEFT, TURN_RIGHT, STOP)
    - "high": High-level actions (goto, explore, open, stop)
    
    Supports:
    - Custom API endpoints (local LLM servers, etc.)
    - OpenAI API (GPT-4, GPT-4o, GPT-3.5-turbo, etc.)
    
    Example with OpenAI (low-level):
        agent = LLMAgent(api_type="openai", model_name="gpt-4o", action_level="low")
    
    Example with high-level actions:
    ...
    def _parse_high_level_action(self, response: str) -> Optional[HighLevelAction]:
        """Parse high-level action from LLM response."""
